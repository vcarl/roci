...
- `--conditions=roci-src` is a **process-global** switch, correct only for a runtime that runs `@roci/core` *from source*. It is needed by **no wired entrypoint**: `@roci/core`'s `exports` map points every subpath to `dist/` with no src condition, so the `./roci` launcher, the `apps/roci` scripts, and the domain packages all consume core **from dist** and resolve `#brain/*` via `default` → `dist/brain/*` with **no flag** (adding the flag would make dist files jump to src). The only source-loading runtime is `vitest` (core's own tests), handled by the `resolve.alias` mirror above, not the condition. The `roci-src` branch remains only as an escape hatch for direct `node --import tsx <core-src>.ts` debugging. *(Corrects an earlier draft that told every "from-source runtime" to add the flag — the launchers are dist consumers; verified against `packages/core/package.json` `exports` and the `apps/roci → packages/core` symlink.)*
