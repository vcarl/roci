...
### 2g. Command codec + NDJSON row codec seam (decision 2026-07-23)
The `memory` CLI's **arg grammar** and its **NDJSON output contract** are now single-owned in the package, closing the last two places host and container could drift.

- **`command-codec.ts`** owns the full verb/flag grammar as one module: `encode{Remember,Search,Recent,MarkGet,MarkSet,Promote}Args` (host → argv) and `parse{Remember,Search,Recent}Args` / `parseCommand` (container argv → typed command). Grammar lived in two places before — `longterm-store.ts`'s hand-concatenation and the CLI's own parser — and the tested `parseMemoryArgs` mirror modelled *neither* fully (it had no `--dims`, no `--source`/`-k`/`-n` defaults; phase-1 report flagged this). `parseMemoryArgs` is **deleted into** the codec; `memory-args.ts` is gone.
  - **Defaults live in the parser:** `--source` → `"conscious"`, `-k` → 5, `-n` → 10.
  - **`--dims` semantics preserved exactly:** empty/absent signature ⇒ the encoder OMITS the flag ⇒ NULL column ⇒ neutral salience at recall. A present-but-malformed `--dims` is a **hard parse error** (stderr + non-zero exit), never a silently-stored corrupt signature.
- **Core is grammar-blind.** `longterm-store` maps `shQuote` over the codec's argv (`buildMemoryCommand`). The raw command string is therefore uniformly quoted rather than per-value quoted — cosmetically different from the legacy hand-concat but **provably shell-equivalent**: bash word-splits both to the identical `process.argv` the deployed string-CLI reads. The compatibility constraint (encoder switched in NOW, deployed parser unchanged until phase 3) is gated by a **bash-splitter diff test** (`longterm-store.test.ts`) over adversarial entries (quotes, unicode, newline, empty/absent dims).
- **NDJSON row codec.** The host-side recall parse (split-lines + `JSON.parse` + drop-on-error) moved into `memory-format.ts` next to `formatResults` as `parseResults`, round-trip-tested against `formatResults`'s exact shape (dims-as-object, tags-as-array, score presence). **Loudness change:** a torn line is now **logged** (`console.warn` hook, injectable) *before* being dropped — no longer fully silent.
...
> **IMPLEMENTED 2026-07-23.** `provisionMemoryCli` reads the bundle via the package boundary (`readPlayerToolBundle`, `createRequire` of `@roci/player-tools/package.json`); `longterm-store` delivers the host-rewritten `MEMORY_EMBED_URL` via an `export …&&` prefix on all five exec calls (`Docker.exec` has no `-e` seam). OLD-vs-NEW byte-diff gate + live provision-path re-validation both PASS (phase3-gate-evidence.md).
...
- **Validate — byte-diff NDJSON against a live container run is MANDATORY before the old path is deleted:** in a real container, run `memory remember` + `memory search` + `memory recent` + `promote`/`mark-get`/`mark-set` through *both* the old string binary and the new bundle against the same db fixture, and assert **byte-identical NDJSON** (field order, `score` formatting, empty-result empty string, trailing-newline behavior) — this is the `longterm-store.ts:194-205` parse contract. Also verify the vec0 load, the migration `ADD COLUMN` loop on a legacy db, and the cold-start retry (kill the embed server, confirm the new binary retries where the old one threw). Only on a clean diff does phase 3 land.
  - **Two MANDATORY codec byte-diff cases** (added with the 2026-07-23 codec seam, §2g) — the host-side compat test proves argv shell-equivalence, but the *round-trip through the real deployed parser* must be checked live: (1) **`remember --dims` write** — a `remember` carrying a non-empty dims signature stores the `dims` column identically (and the codec's dims-omitted-when-empty rule yields NULL exactly as before); (2) **`search` returning dims rows** — a `search` whose hits carry a non-null `dims` column emits byte-identical NDJSON (dims-as-object) through both binaries, since `parseResults` now owns that parse. A malformed-`--dims` invocation must also be confirmed to hard-error (non-zero exit) on the new binary.
- **Rollback:** provisioning flips back to `build*CliScript` (still present); one-line revert.

### Phase 4 — `wm` rides along
> **IMPLEMENTED 2026-07-23.** `provisionWmCli` reads the `wm` bundle via `readPlayerToolBundle`; no per-run env (store paths are cwd-relative + baked). wm phase-4 pre-evidence BYTE/STRUCTURAL-IDENTICAL + live `wm todo` re-validation PASS.
...
> **IMPLEMENTED 2026-07-23.** `buildMemoryCliScript`/`buildWmCliScript` + the generation-only constants (`MemoryCliOpts`, `VEC_EXTENSION_PATH`, `DEFAULT_DB_PATH`, core `WM_USAGE`/`WM_JSON_REL`/`WM_MD_REL`) and their string-scraping test suites are gone; the two core CLI files are now provisioning-only. The `.toString()` embedding + the OLD-vs-NEW byte-diff harness path are removed (the harness is now provision-path-only). `knnSql`/`fmt`/`splitTags`/`takeFlag`/`intOr`/`classify`/duplicate-`embed` never existed in core after phase 1 (they were the deleted-into-package hand-copies).
