...
/** Path to the hippocampus dream/cull prompt templates (consolidate.md now lives here too). */
	...
	execute: (input: DreamInput) =>
		Effect.gen(function* () {
			...
			if (preConsolidateDiary.length < DIARY_MIN_COMPRESS_CHARS) {
				...
				const consolidateTemplate = yield* loadTemplate(path.join(PROMPTS_DIR, "consolidate.md"))
