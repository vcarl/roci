...
export function makeMlxBackend(
  ...
  return Effect.gen(function* () {
    ...
    // Default spawn seam: resolve mlx_lm.server from the venv (or PATH), start
    // it, and adapt the platform Process to our SpawnedProcess shape (pid +
    // stderr stream). Tests inject their own seam, so the venv/FS resolution
    // lives here, in the default. The pure resolver (resolveMlxCommand) is unit-
    // tested separately without spawning or touching the real filesystem.
    const startProcess =
      deps.startProcess ??
      ((spec: TierSpec): Effect.Effect<SpawnedProcess, unknown, Scope.Scope> =>
        Effect.suspend(() => {
          ...
          return executor.start(command).pipe(
            ...
            // convert it to the same actionable missing-runtime message. We show
            // the directory the binary was resolved from (the venv bin dir, or
            // the bare command's dir).
            Effect.mapError((e) =>
