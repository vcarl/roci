...
describe("MODEL_TIER_SPECS", () => {
  it("pins conscious to the resident gpt-oss-20b GGUF (llamacpp) on port 8083", () => {
    const c = MODEL_TIER_SPECS.conscious
    expect(c.model).toBe("unsloth/gpt-oss-20b-GGUF")
    expect(c.provider).toBe("llamacpp")
    expect(c.port).toBe(8083)
    expect(c.lifecycle).toBe("resident")
    expect(c.baseUrl).toBe("http://127.0.0.1:8083/v1")
  })
  it("makes forebrain the per-phase 9B on 8082 and hindbrain the per-phase 2B on 8081, both mlx", () => {
    expect(MODEL_TIER_SPECS.forebrain.model).toBe("mlx-community/Qwen3.5-9B-4bit")
    expect(MODEL_TIER_SPECS.forebrain.provider).toBe("mlx")
    ...
    expect(MODEL_TIER_SPECS.hindbrain.provider).toBe("mlx")
  ...
  it("derives model and baseUrl from handles for every tier (single source of truth)", () => {
    for (const tier of ["hindbrain", "forebrain", "conscious"] as const) {
      ...
      expect(spec.provider).toBe(handle.provider)
    }
  })
})
