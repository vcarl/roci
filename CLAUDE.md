This project is a general-purpose agent orchestrator that runs autonomous character-driven sessions using Claude Code as the agent runtime. The core architecture is domain-agnostic: a state machine event loop, brain/body execution model, and 6 injectable Effect service layers handle all domain-specific behavior. Characters have persistent identities (background, values, secrets, diary) and operate inside a shared Docker container.
