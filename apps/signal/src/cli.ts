...
import { createGitHubApp } from "@signal/domain-github/create-app.js"
import { writeFileSync, readFileSync, existsSync, mkdirSync } from "node:fs"
import { Docker, DockerLive } from "@signal/core/services/Docker.js"
import { CharacterFs, CharacterFsLive, makeCharacterConfig } from "@signal/core/services/CharacterFs.js"
import { ClaudeLive } from "@signal/core/services/Claude.js"
import { CharacterLogLive } from "@signal/core/logging/log-writer.js"
...
import { OAuthTokenLive } from "@signal/core/services/OAuthToken.js"
...
const nonstopOption = Options.boolean("nonstop").pipe(
  Options.withDefault(false),
  Options.withDescription("Restart characters automatically after wind-down (waits for session reset, then relaunches)"),
)

// --- start command ---
const startCharacters = Args.text({ name: "characters" }).pipe(Args.repeated)

const startCommand = Command.make("start", { characters: startCharacters, tickInterval, domain: domainOption, manualApproval, nonstop: nonstopOption }, (args) =>
  Effect.gen(function* () {
    ...
    yield* runOrchestrator(resolved, args.tickInterval, args.manualApproval, args.nonstop)
  }),
...
const defaultNonstop = Options.boolean("nonstop").pipe(
  Options.withDefault(false),
  Options.withDescription("Restart characters automatically after wind-down (waits for session reset, then relaunches)"),
)
...
const runAutoDetect = (args: {
  ...
  Effect.gen(function* () {
    ...
    yield* validateAndStart(PROJECT_ROOT, resolved, args.tickInterval, args.manualApproval, args.nonstop)
  })
