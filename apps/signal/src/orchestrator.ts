...
import { Docker, DockerError } from "@signal/core/services/Docker.js"
import { runPhases } from "@signal/core/core/phase-runner.js"
...
import { makeCharacterConfig } from "@signal/core/services/CharacterFs.js"
 ...
 */
export const runOrchestrator = (resolvedDomains: ResolvedDomain[], tickIntervalSeconds: number, manualApproval = false, nonstop = false) =>
  Effect.gen(function* () {
    ...
    for (const rd of resolvedDomains) {
      ...
      for (const charName of rd.characters) {
        ...
        // Inline character loop: scoped phase runner (single run)
        const singleRun = Effect.scoped(
          Effect.gen(function* () {
        ...
        const loopEffect = nonstop
          ? Effect.gen(function* () {
              ...
              while (true) {
                iteration++
                if (iteration > 1) {
                  yield* logToConsole(charName, "orchestrator", `Nonstop: restarting (iteration ${iteration})...`)
                }
                yield* singleRun
                ...
                if (windDown?.sessionEndTime) {
                  ...
                  yield* logToConsole(charName, "orchestrator", "Nonstop: early halt. Restarting in 60s...")
                  yield* Effect.sleep(Duration.seconds(60))
                }
                // Clear wind-down so the next session starts fresh
