 ...
 * Responds to HELP_REQUESTED signals with a quick Claude -p call.
 *
 * Usage:
 *   node dist/overmind/overlord.js [--players-dir <path>] [--interval <seconds>]
 *
 * Depends on:
 *   @roci/core/operator/wind-down-file  (PR: feat/operator-wind-down)
 *   @roci/core/operator/edict-inbox     (PR: feat/operator-ipc)
 *   @roci/core/server/status-reporter   (PR: feat/status-reporting)
 */

import * as fs from "node:fs"
import * as path from "node:path"
import { execSync } from "node:child_process"
import { readWindDown } from "@roci/core/operator/wind-down-file.js"
import { writeEdict } from "@roci/core/operator/edict-inbox.js"
import type { AgentStatusSnapshot } from "@roci/core/server/status-reporter.js"
...
function readAllStatus(): Map<string, AgentStatusSnapshot> {
  ...
  // Agents not updated within 3× poll interval are considered offline
...
function processStuck(name: string, snapshot: AgentStatusSnapshot): void {
  ...
  if (stuckCount >= STUCK_THRESHOLD) {
    ...
    const nudgeCooldownMs = DEFAULT_POLL_INTERVAL_MS * STUCK_THRESHOLD * 2
    if (Date.now() - lastNudge > nudgeCooldownMs) {
      console.log(`[Overlord] ${name} stuck (phase=${snapshot.phase}, step=${snapshot.stepIndex}, count=${stuckCount}) — nudging`)
      const playerDir = path.join(playersDir, name)
      writeEdict(playerDir, {
        priority: "low",
        content: `You appear to be stuck on [${snapshot.currentGoal ?? "unknown goal"}] in phase ${snapshot.phase} for ${stuckCount * Math.round(pollIntervalMs / 60000)} minutes. Assess your situation and either continue or replan.`,
...
function handleHelpRequest(req: HelpRequest, allStatus: Map<string, AgentStatusSnapshot>): void {
  ...
  const prompt = `You are the Overlord, the fleet's autonomous monitor.
...
Provide a focused, actionable response as a high-priority edict.
Be direct. One to three sentences max.
Respond with only the edict text — no preamble.`
  ...
  try {
    ...
    if (response) {
      ...
      writeEdict(playerDir, {
        priority: "high",
        content: `[OVERLORD RESPONSE]\n${response}`,
        issuedAt: new Date().toISOString(),
        source: "overlord",
      })
      console.log(`[Overlord] Responded to ${req.agentName}: ${response.slice(0, 100)}`)
    }
  } catch (err) {
...
async function poll(): Promise<void> {
  ...
  // Skip if wind-down already active
  const existingWindDown = readWindDown(playersDir)
  if (existingWindDown) {
    console.log(`[Overlord] Wind-down active: ${existingWindDown.reason}`)
    return
  }

  // 1. Read all agent status
  const allStatus = readAllStatus()
  console.log(`[Overlord] Active agents: ${[...allStatus.keys()].join(", ") || "none"}`)

  // 2. Drain help requests
